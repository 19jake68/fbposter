    ...
    if ( $db = new PDO( 'sqlite:' . $dbName . '-crons.db' ) ) {
            ...
            if ( $_POST[ 'del' ] == 1 ) {
                if ( isset( $_POST[ 'cronid' ] ) ) {
                    ...
                    if ( $adminloggedIn || ( "$fullname ($user)" == $userIdentifier ) ) {
                        if ( $adminloggedIn && ( $cronid == "ALL" ) && ( $userIdentifier == "--ALL--" ) ) {
                            $db = null;
                            unlink( $dbName . '-crons.db' );
                        } elseif ( $cronid == "ALL" )
                            $statement = $db->prepare( "DELETE FROM Crons WHERE user=\"$userIdentifier\"");                        
                        else
        ...
        function Accounts(e, pid, t) {
            ...
            if (t==1) {
                ...
                $('.confirm').easyconfirm({
                    eventType: 'submit',
                    locale: { title: '" . $lang['Removing CRON Post'] . "', text: '" . $lang['Are you sure'] . " " . $lang['Remove'] . " " . $lang['All'] . "', button: ['" . $lang['Cancel'] . "','" . $lang['Remove'] . "']}
                });
                document.forms['Account'].del.value=1;
            } else if (t==3) {
                $('.confirm').easyconfirm({
                    eventType: 'submit',
                    locale: { title: '" . $lang['Clear'] . " " . $lang['All'] . "', text: '" . $lang['Are you sure'] . " " . $lang['Clear'] . " " . $lang['All'] . "', button: ['" . $lang['Cancel'] . "','" . $lang['Clear'] . "']}
                });
                document.forms['Account'].del.value=1;
            }
